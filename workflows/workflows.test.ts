...
import { harnessOf, type ActivityLike } from "../src/agent/camunda7.ts";
...
describe.each(files)("%s", (file) => {
  ...
  it("gives every service task a harness", async () => {
    const elements = await elementsOf(file);
    const serviceTasks = elements.filter((e) => e.$type === "bpmn:ServiceTask");
    for (const task of serviceTasks) {
      expect(harnessOf(asActivity(task)), `${task.id} has no harness`).toBeTruthy();
    }
  });
  ...
  it("writes conditions in FEEL, not JUEL or Python", async () => {
    const elements = await elementsOf(file);
