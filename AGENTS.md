...
- **Follow the running server through `watch.log`, at the repo root.** `make watch` tees its
  ...
  from `app.log`, the app's own `configure_logging()` output (`bpmn_agent/logging_config.py`): that
...
- **Offline demo**: `devenv shell -- demo` runs uvicorn with `PI_EXECUTABLE=bpmn_agent/data/pi-demo`.

## 2. Local Pi Agent & Deterministic Demo

- **Executable Fallback**: `PI_EXECUTABLE` defaults to `node_modules/.bin/pi` in devenv. Falls back to `bpmn_agent/data/pi-demo` when `PI_OFFLINE=1`, `.pi_offline` file exists, or no `OPENAI_API_KEY` is set.
- **Demo fallback is opt-in**: a *failed* real Pi run only retries against `bpmn_agent/data/pi-demo` when `PI_ALLOW_DEMO_FALLBACK=1`. Off by default — otherwise a misconfigured provider silently feeds fabricated agent output into BPMN gateway conditions.
- **Deterministic Showcase**: `bpmn_agent/data/pi-demo` is a fast RPC-compatible mock that always emits the 5-key JSON result contract without model credentials.
...
- **Task scope vs. process scope (Camunda's task-scope-variables model)**: the raw,
  ...
  `bpmn_agent/orchestration/jobs.py`, surfaced as the `jobs` field in the API/UI), *never* on
...
- **Reading data into a prompt (inputParameters)**: `camunda:inputParameter` expressions are
  resolved by `resolve_input()` (`bpmn_agent/engine.py`), a pure dict/list lookup + string
...
- **Projects API**: `ProjectService` (`bpmn_agent/projects.py`) is a read/write projection over
...
- **Adapter**: `ShellAdapter` (`bpmn_agent/adapters/shell_adapter.py`), registered as `harness_type` `shell`. This is the non-LLM half the adapter registry exists for — a compiler, slicer, or CAM step is a BPMN node like any other.
...
- **Output sanitization**: Agent output strings truncated at 50 000 characters (`_sanitize_output` in `bpmn_agent/orchestration/jobs.py`).
...
1. Subclass `bpmn_agent.adapters.base.BaseAdapter`; implement `adapter_type` property and `run(prompt, config, cwd) -> AgentResult`. Override `prepare_workspace(workdir, config)` if the harness needs configuration on disk before the turn (as `SandboxPiAdapter` does for its network policy).
