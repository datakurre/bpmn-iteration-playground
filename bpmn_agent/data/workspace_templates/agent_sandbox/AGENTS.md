...
`prepare_sandbox_workspace()` (`bpmn_agent/adapters/sandbox_policy.py`), used by both
...
`bpmn_agent/adapters/sandbox_adapter.py`), so the proxy only ever needs to inject one value.
