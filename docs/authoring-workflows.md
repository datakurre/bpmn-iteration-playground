...
All executable workflows are standard BPMN 2.0 XML files placed in `bpmn_agent/data/workflows/`.
...
instance-wide data. `bpmn_agent/data/workflows/composed_delivery.bpmn` calling `bpmn_agent/data/workflows/agent_review_cycle.bpmn`
...
payload lands on the new child's data. `bpmn_agent/data/workflows/project.bpmn` is a working example.
...
parking the instance in `failed`. `bpmn_agent/data/workflows/beamer_slides.bpmn` is a working example — a
