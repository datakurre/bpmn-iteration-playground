...
export function measureTextWidth(text: string, fontSize: number): number {
  ...
  for (let i = 0; i < text.length; i++) {
    const ch = text[i];
    const ratio = (ch ? ARIAL_CHAR_RATIOS[ch] : undefined) ?? DEFAULT_CHAR_RATIO;
...
function parseTransformTranslation(attr: string | null): { tx: number; ty: number } {
  ...
  if (translateMatch) {
    const tx = parseFloat(translateMatch[1] ?? '0') || 0;
  ...
  if (matrixMatch) {
    const tx = parseFloat(matrixMatch[5] ?? '0') || 0;
    const ty = parseFloat(matrixMatch[6] ?? '0') || 0;
    return { tx, ty };
  }
