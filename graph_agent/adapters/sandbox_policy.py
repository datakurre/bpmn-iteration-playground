...
# Two distinct roots: PACKAGE_ROOT is where this package's own bundled data lives (ships
# inside the wheel, works installed or from a source checkout); REPO_ROOT is this repo's
# checkout root, needed only to locate the vendored vendor/agent-sandbox submodule below --
# a developer-machine concern, not something an installed package carries with it.
PACKAGE_ROOT = Path(__file__).resolve().parents[1]
REPO_ROOT = PACKAGE_ROOT.parent
TEMPLATE_ROOT = PACKAGE_ROOT / "data" / "workspace_templates"
