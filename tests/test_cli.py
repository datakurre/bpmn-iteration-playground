...
def test_cli_serve_defaults_log_file_under_agents_logs(
    ...
    forever as untracked cruft, permanently failing `bpmn merge`'s clean-working-tree
    precondition (workspace_strategy.py's WorktreeStrategy.merge) -- it must default under
    `.agents/logs/`, never logging_config.py's own CWD-relative default."""
    os.environ.pop("LOG_FILE", None)
    workspace = Workspace.discover(tmp_path)
    ...
    assert os.environ["LOG_FILE"] == str(workspace.logs_dir / "bpmn-agent.log")
