...
def test_history_instances_listing_and_deletion(client: TestClient) -> None:
    ...
    start_resp = client.post(
        "/workflow/start",
        json={"bpmn_path": "bpmn_agent/data/workflows/contract_review.bpmn", "variables": {"contract": "History test"}},
    )
    ...
    client.post("/workflow/start", json={"bpmn_path": "bpmn_agent/data/workflows/contract_review.bpmn", "variables": {}})
...
def test_history_instances_pagination_and_date_filtering(client: TestClient) -> None:
    ...
    for i in range(5):
        resp = client.post(
            "/workflow/start",
            json={"bpmn_path": "bpmn_agent/data/workflows/contract_review.bpmn", "variables": {"idx": i}},
        )
