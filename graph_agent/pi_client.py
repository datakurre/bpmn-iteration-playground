...
class PiClient:
    ...
    def __init__(
        ...
        # demo_executable ships as package data (works installed or from a checkout);
        # root_dir is the repo checkout root, still needed for the node_modules-relative
        # dev fallback and for resolving a relative PI_EXECUTABLE the way devenv sets it.
        root_dir = Path(__file__).resolve().parents[1]
        demo_executable = Path(__file__).resolve().parent / "data" / "pi-demo"
    ...
    async def _execute(  # noqa: C901, PLR0913, PLR0915 -- subprocess spawn/stream/timeout/cancel/parse lifecycle; pre-existing complexity
        ...
        # touching that machinery. See graph_agent.workspace._run_tar for the same fix
