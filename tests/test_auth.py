...
def test_rbac_with_api_keys(client: TestClient, monkeypatch) -> None:
    ...
    assert client.post("/workflow/start", json={"bpmn_path": "bpmn_agent/data/workflows/contract_review.bpmn"}).status_code == 401
    ...
    assert client.post("/workflow/start", json={"bpmn_path": "bpmn_agent/data/workflows/contract_review.bpmn"}, headers=viewer_headers).status_code == 403

    # 4. Operator key can start workflows and submit tasks
    op_headers = {"X-Api-Key": "op-key"}
    start_resp = client.post("/workflow/start", json={"bpmn_path": "bpmn_agent/data/workflows/contract_review.bpmn"}, headers=op_headers)
...
def test_require_auth_fails_when_unconfigured(client: TestClient, monkeypatch) -> None:
    ...
    resp = client.post("/workflow/start", json={"bpmn_path": "bpmn_agent/data/workflows/contract_review.bpmn"})
